...
import entity.tower.Tower; 
 ...
 /**
 * The class Playing extends game scene implements scene methods
 */ 
public class Playing extends GameScene implements SceneMethods {

	/**
	 * X (pixel) position of mouse
	 */
	private int mouseX;
	/**
	 * Y (pixel) position of mouse
	 */
	private int mouseY;
	/**
	 * level grids.
	...
	private void drawLevel(GraphicsContext gc) { 
		...
		for (int y = 0; y < lvl.length; y++) {
			for (int x = 0; x < lvl[y].length; x++) {
				int id = lvl[y][x];
				if(isAnimation(id)) {
					gc.drawImage(getSprite(id, getAnimationIndex()), x * 32, y * 32);
				}
				else {
 ...
 */
	...
	/**
	 * set the pause status of the getGame().
	 */
